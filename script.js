...
// ===================== Google Analytics: mapeamento de eventos =====================
...
function gaLabel(el) {
  return (el.getAttribute("data-ga") || el.getAttribute("aria-label") || (el.textContent || "").trim()).slice(0, 80);
}

function gaSection(el) {
  const section = el.closest("section, footer, header");
  return section ? section.id || section.className.split(" ")[0] || section.tagName.toLowerCase() : "pagina";
}

function initAnalytics() {
  // Cliques em WhatsApp (CTAs de demonstração e contato)
  document.querySelectorAll('a[href*="wa.me"]').forEach((link) => {
    link.addEventListener("click", () => {
      const text = (link.textContent || "").toLowerCase();
      const isDemo = /demonstra|agendar|solicitar|demo|em ação/.test(text);
      if (isDemo) {
        track("gerar_lead", { method: "whatsapp", event_category: "conversao", event_label: gaLabel(link), location: gaSection(link) });
      }
      track("whatsapp_click", { event_category: "contato", event_label: gaLabel(link), location: gaSection(link) });
    });
  });

  // Navegação interna (âncoras do topo e rodapé)
  document.querySelectorAll('a[href^="#"]').forEach((link) => {
    link.addEventListener("click", () => {
      track("nav_click", { event_category: "navegacao", event_label: gaLabel(link), destino: link.getAttribute("href"), location: gaSection(link) });
    });
  });
  ...
  document.querySelectorAll(".footer-contact a").forEach((link) => {
    link.addEventListener("click", () => {
      const tipo = link.href.indexOf("tel:") === 0 ? "telefone" : link.href.indexOf("wa.me") > -1 ? "whatsapp" : "site";
      track("contact_click", { event_category: "contato", event_label: tipo, link_url: link.href });
    });
  });

  // Links externos genéricos
  document.querySelectorAll('a[href^="http"]').forEach((link) => {
    if (link.href.indexOf("wa.me") > -1 || link.closest(".footer-contact")) return;
    link.addEventListener("click", () => {
      track("outbound_click", { event_category: "saida", event_label: gaLabel(link), link_url: link.href });
    });
  });

  // Interações com os mockups (perguntas do hero, perguntas da IA e KPIs do Command Center)
  document.querySelectorAll("[data-hero-question]").forEach((button) => {
    button.addEventListener("click", () => {
      track("demo_interaction", { event_category: "engajamento", event_label: button.getAttribute("data-hero-question"), area: "hero" });
    });
  });
  document.querySelectorAll(".question-option[data-question]").forEach((button) => {
    button.addEventListener("click", () => {
      track("demo_interaction", { event_category: "engajamento", event_label: button.getAttribute("data-question"), area: "ia" });
    });
  });
  document.querySelectorAll("[data-command-metric]").forEach((button) => {
    button.addEventListener("click", () => {
      track("demo_interaction", { event_category: "engajamento", event_label: button.getAttribute("data-command-metric"), area: "command-center" });
    });
  });

  // Visualização de cada seção (engajamento por bloco)
  const viewedSections = {};
  const sectionObserver = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          const id = entry.target.id || "sec";
          if (!viewedSections[id]) {
            viewedSections[id] = true;
            track("view_section", { event_category: "engajamento", event_label: id });
          }
  ...
  document.querySelectorAll("section[id], footer").forEach((section) => sectionObserver.observe(section));
    ...
    () => {
      ...
      milestones.forEach((mark) => {
        if (pct >= mark && !reached[mark]) {
          reached[mark] = true;
          track("scroll_depth", { event_category: "engajamento", event_label: mark + "%", percent: mark });
        }
      });
    },
    { passive: true }
